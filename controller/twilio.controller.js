...
class TwilioController {
    static requestLoginOTP = async (req, res) => {
        ...
        try {
            // 2. Service Call
            const data = await TwilioServices.sendOTP(phone);
    ...
    static verifyLoginOTP = async (req, res) => {
        try {
            ...
            const check = await TwilioServices.verifyOTP(phone, otpCode);

            if (check.status !== 'approved') {
                return res.status(400).json({ success: false, message: "Invalid or expired OTP." });
            }

            const session = await TwilioServices.handleUserSession(phone);
    ...
    static completeProfile = async (req, res) => {
        try {
            ...
            const session = await TwilioServices.completeUserProfile(
                req.user.id,
                { name, gender },
                coordinates
            );
